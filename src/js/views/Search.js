...
class Search extends React.Component {
  ...
  search = (force = false) => {
    const {
      coreActions: { startSearch },
      uiActions: { setWindowTitle },
      search_results_query: { type: existingType, term: existingTerm },
      type,
    } = this.props;
    const { term } = this.state;

    setWindowTitle(
      i18n("search.title_window", { term: decodeURIComponent(term) })
    );

    if (
      type &&
      term &&
      (force || existingType !== type || existingTerm !== term)
    ) {
  ...
  render = () => {
    ...
    const options = (
      <>
        <DropdownField
          ...
          selected_icon={
            sort_reverse ? "keyboard_arrow_up" : "keyboard_arrow_down"
          }
    ...
    switch (type) {
      case "artists":
        searchResults = (
          <SearchResults type="artists" query={{ term, type: "artists" }} />
        );
        break;
      case "albums":
        searchResults = (
          <SearchResults type="albums" query={{ term, type: "albums" }} />
        );
        break;
      case "playlists":
        searchResults = (
          <SearchResults type="playlists" query={{ term, type: "playlists" }} />
        );
        break;
      case "tracks":
        searchResults = (
          <SearchResults type="tracks" query={{ term, type: "tracks" }} />
        );
        break;
      default:
        searchResults = (
    ...
    return (
      <div className="view search-view">
        ...
        <div className="content-wrapper">{searchResults}</div>
      </div>
    );
  };
}
...
const mapStateToProps = (state, ownProps) => {
  ...
  const {
    ...
    core: { search_results: { query: search_results_query = {} } = {} },
  } = state;
