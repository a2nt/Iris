...
const Hotkeys = () => {
  ...
  const play_time_position = useSelector((state) =>
    parseInt(state.mopidy.time_position, 10)
  );
  ...
  const show_disconnected_clients = useSelector(
    (state) => state.ui?.snapcast_show_disconnected_clients
  );
  const prepare = ({ e, label, callback }) => {
    const { target, altKey, ctrlKey, metaKey, shiftKey, key } = e;
  ...
  const setSnapcastVolume = (index, adjustment) => {
    ...
    const groupVolume =
      groupClients.reduce((acc, client) => acc + (client.volume || 0), 0) /
      groupClients.length;

    if (group.mute) snapcastActions.setGroupMute(group.id, false);
    dispatch(
      snapcastActions.setGroupVolume(
        group.id,
        groupVolume + adjustment,
        groupVolume
      )
    );
    dispatch(
