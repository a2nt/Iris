...
const App = () => {
  ...
  const wide_scrollbar_enabled = useSelector(
    (state) => state.ui.wide_scrollbar_enabled
  );
  const hide_scrollbars = useSelector((state) => state.ui.hide_scrollbars);
  const smooth_scrolling_enabled = useSelector(
    (state) => state.ui.smooth_scrolling_enabled
  );
  ...
  const initial_setup_complete = useSelector(
    (state) => state.ui.initial_setup_complete
  );
      ...
      if (allow_reporting) {
        ...
        Sentry.init({
          ...
          beforeSend: (event, hint) => {
            const { originalException: { message } = {} } = hint;
            ...
            if (
              ...
              (message.match(/Websocket/i) ||
                ...
                message.match(
                  /Non-Error promise rejection captured with keys: call, message, value/i
                ) ||
