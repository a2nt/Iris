...
function setCookie(key, value, exdays = -1) {
  ...
  document.cookie =
    key + "=" + value + ";" + expires + ";sameSite=Strict;path=/";
}
...
class TimeProvider {
  ...
  now() {
    if (!this.ctx) {
      ...
      const contextTime = !!this.ctx.getOutputTimestamp
        ? this.ctx.getOutputTimestamp().contextTime
        : undefined;
      return (
        (contextTime !== undefined ? contextTime : this.ctx.currentTime) * 1000
      );
    }
  }
  nowSec() {
